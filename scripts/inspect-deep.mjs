...
async function 팝업조사(팝업접두, 자리이름, 돌아갈주소, 메모) {
  ...
  /**
   * 이미 팝업을 잡은 글자. **목록은 줄마다 같은 버튼이 있습니다**(상품목록의 `수정` 은 19개).
   * 19개가 다 같은 팝업을 엽니다. 한 번 잡았으면 나머지는 안 눌러도 됩니다.
   * (실측 2026-08-04: 19번 눌러서 얻은 것은 팝업 2개였고, 나머지 17번은 "사라졌습니다" 로 헛돌았습니다.)
   */
  const 잡은글자 = new Set();
  let 건너뛴수 = 0;
  ...
  for (const [순번, 글] of 열것글자.entries()) {
    ...
    try {
      ...
      팝업번호++;
      잡은글자.add(글); // 같은 글자는 이제 안 누릅니다(줄마다 있는 같은 버튼이라 팝업도 같습니다).
  ...
  // 빼먹은 것을 조용히 넘어가지 않습니다. 몇 개를 왜 안 눌렀는지 적습니다.
  if (건너뛴수) console.log(`   같은 글자 ${건너뛴수}개는 안 눌렀습니다(줄마다 있는 같은 버튼 — 팝업이 같습니다)`);
}
